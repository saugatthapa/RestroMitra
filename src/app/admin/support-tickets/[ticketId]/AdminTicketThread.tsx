            ...
            <div className="flex flex-wrap gap-1.5">
              {(["open", "in_progress", "resolved", "closed"] as TicketStatus[]).map((s) => (
                <button
                  ...
                  className="rounded-md border border-hairline px-2.5 py-1 text-xs text-ink-secondary hover:bg-surface-1 disabled:cursor-not-allowed disabled:opacity-40"
                >
            ...
            {messages.map((m) => (
              <div
                ...
                className={`max-w-[85%] rounded-lg border p-3 text-sm ${
                  m.isFromPlatform
                    ? "ml-auto border-orange-100 bg-orange-500/15"
                    : "ml-0 border-hairline/60 bg-surface-1"
                }`}
              >
