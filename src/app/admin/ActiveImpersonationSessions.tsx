...
export function ActiveImpersonationSessions() {
  ...
  return (
    <div className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/15 p-4">
      ...
      <div className="space-y-2">
        {sessions.map((s) => (
          <div
            key={s.id}
            className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-amber-500/30 bg-surface-2 px-3 py-2 text-sm"
          >
            <div>
              <p className="font-medium text-ink">
                ...
                <span className="text-xs text-ink-muted">({s.mode === "write" ? "read/write" : "read-only"})</span>
              </p>
            ...
            <button
              ...
              onClick={() => revoke(s.id)}
              className="inline-flex items-center justify-center rounded-lg border border-red-500/30 bg-surface-2 px-3 py-1.5 text-xs font-semibold text-red-400 transition hover:bg-red-500/15 disabled:cursor-not-allowed disabled:opacity-60"
            >
