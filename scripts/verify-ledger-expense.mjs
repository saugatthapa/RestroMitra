...
await page.fill('input[placeholder="e.g. Electricity bill for Shrawan"]', "Test ledger-hook expense");
await page.locator('input[type="number"]').fill("777");
await Promise.all([
  page.waitForResponse((r) => r.url().endsWith("/expenses") && r.request().method() === "POST"),
